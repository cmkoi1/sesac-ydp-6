...
app.set('view engine', 'ejs'); // 뷰 엔진이 ejs임을 알려 줌
...
// -> post 형태로 들어오는 모든 형식의 데이터를 파싱
app.use(express.json()); // 요청의 body 객체에 json 형태로(일단은 js object와 유사한 것으로 알고 있자...)로 온다.

// 라우팅(Routing) => 주소를 설정한다.
// localhost:PORT/ 경로로 접속했을 때, index.ejs를 응답하겠다.
// 브라우저에서 엔터를 칠 경우 -> get 방식
app.get('/', (req, res) => {
    res.render('index', { title: '폼 실습을 해봅시다'});
})

app.get('/getForm', (req, res) => {
    console.log(req.query); // { id: 'banna', pw: '1234' }
    // res.send('get 요청 성공');
...
app.post('/postForm', (req, res) => {
    // body-parser가 없을 경우 undefined로 나온다.
    console.log(req.body); // body-parser가 있을 경우 { id: 'banna', pw: '1234' }
    // res.send('post 요청 성공');
...
// res.render(뷰, 데이터): 템플릿 엔진을 사용해서 서브 측에서 "동적으로" HTML을 생성하고 클라이언트에게 응갑
