...
select * from customer where phone not like '%_4'; -- %4 x

-- 6. order by: 정렬
select * from customer order by custname; -- defrault asc
select * from customer order by custname desc;

-- 주의) order by 절은 where 절보다 뒤에 위치해야 함
...
select custid, sum(amount), sum(price * amount) 
	from orders
		where custid != 'bunny'
        group by custid having sum(price * amount) >= 100000;
...
- 각각의 행을 필터링
- group by보다 앞에 위치
- 집계함수를 쓸 수는 있으나 having절보다는 자유롭지 못함 
*/