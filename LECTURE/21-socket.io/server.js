  ...
  // [실습 4] 채팅창 메시지 전송
    ...
    socket.on('send', (data) => {
        ...
        if(data.dm === "all") {
            ...
            io.to(dmSocketId).emit('newMessage', sendData) // DM을 보내야 하는 타겟(소켓 아이디)한테 메세지 전송
            socket.emit('newMessage', sendData);
            // console.log("sendData >> ", sendData); // sendData >>  { nick: '2', msg: '1', dm: '(속닥속닥)' }
        }
    })
})
